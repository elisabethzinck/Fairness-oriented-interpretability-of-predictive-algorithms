...
class DescribeData:
    ...
    def agg_table_to_tex(self, target_tex_name = None):
        ...
        df_agg = self.agg_data.sort_values(by = 'N', ascending = False)
        # appending total row: 
    ...
    def plot_positive_rate(self, orientation = 'v', title =None):
        if title is None: 
        ...
        if orientation == 'h':
            ...
            ax.set_xlabel('', fontsize = 12)
            ax.set_ylabel('', fontsize = 12)
            ax.set_yticklabels([self.sens_grps[i].capitalize() for i in range(self.n_sens_grps)])
